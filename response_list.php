...
// Get booking number from GET request
$colname_rs_prev = "-1";
if (isset($_GET['bno'])) {
    $colname_rs_prev = $_GET['bno'];
}

// Corrected query to get unique response details
$query_rs_prev = sprintf(
    "SELECT DISTINCT sr.resno, sr.serdate, sb.bookno AS bno, m.wrk_name, sr.ser_desc, sr.ser_cost
    FROM service_response sr
    JOIN service_book sb ON sr.bno = sb.bookno
    JOIN mechanic m ON sb.mech_code = m.mech_code
    WHERE sb.bookno = %s AND sb.status = 'processed'",
    GetSQLValueString($colname_rs_prev, "int")
);
